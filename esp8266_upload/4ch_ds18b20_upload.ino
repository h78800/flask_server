...
2025/03/05
4ch_ds18b20_upload
基於Python3.0版以上虛擬環境venv使用樹莓派建立一個動態伺服器來接收ESP8266收集的DS18B20溫度數據，並在前端網頁上呈現折線圖
添加WIFI管理及可自訂義上傳UR
1. 初次啟動：
    ◦ 燒錄程式後，ESP8266 會啟動並檢查 Wi-Fi 憑證。
    ◦ 若無憑證，創建名為 "SensorHub" 的 AP。
    ◦ 連接到 "SensorHub"，訪問 192.168.4.1。
    ◦ 配置頁面中：
        ▪ 輸入 Wi-Fi SSID 和密碼。
        ▪ 在 "伺服器 URL" 欄位輸入自訂 URL（例如 http://192.168.1.100:8080/data）。
        ▪ 保存配置。 
2. 後續啟動：
    ◦ 自動連接 Wi-Fi，使用儲存的 URL。
    ◦ 可通過序列埠在 10 秒內修改 URL（備用）。 
3. 重置：
    ◦ 取消註解 wifiManager.resetSettings() 並重新燒錄，以清除 Wi-Fi 和配置。
*/
...
// ✅ 伺服器設定（初始值，可被使用者覆蓋）
String serverName = "http://192.168.2.128:5000/upload"; // 預設 Flask 伺服器上傳端點
...
void setup() {
  ...
  WiFiManagerParameter customServerName("server", "輸入http://伺服器ip位址:5000/upload", serverNameBuffer, 100); // 自訂輸入欄位
